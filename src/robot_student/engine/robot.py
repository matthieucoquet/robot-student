...
class Robot(KinematicRobot):
    ...
    def get_root_state(self, environment_indices: torch.Tensor | None = None, relative: bool = False):
        position = self._entity.get_pos(envs_idx=environment_indices, relative=relative)
        rotation = self._entity.get_quat(envs_idx=environment_indices, relative=relative)
        velocity = self._entity.get_vel(envs_idx=environment_indices)
        angular_velocity = self._entity.get_ang(envs_idx=environment_indices)
